...
class DefaultReadFullTasksUseCase(
    private val taskRepository: TaskRepository,
    private val defaultDispatcher: CoroutineDispatcher
) : ReadFullTasksUseCase {

    override operator fun invoke(): Flow<List<FullTaskModel.FullTask>> =
        taskRepository.readFullTasksByType(setOf(TaskType.RecurringTask, TaskType.SingleTask))
            .map { allTasks ->
                if (allTasks.isNotEmpty()) {
                    withContext(defaultDispatcher) {
                        ...
                        }.awaitAll().filterIsInstance<FullTaskModel.FullTask>()
                    }
                } else emptyList()
            }
